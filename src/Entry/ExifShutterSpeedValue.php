...
class ExifShutterSpeedValue extends SignedRational
{
    ...
    public function getValue(array $options = [])
    {
        ...
        if ($format === 'exiftool') {
            $val = $this->value->getSignedRational();
    ...
    public function toString(array $options = []): string
    {
        ...
        if ($format === 'exiftool') {
            ...
            return MediaProbe::fmt('%.0f/%.0f sec. (APEX: %d)', $this->value->getSignedLong(0), $this->value->getSignedLong(4), pow(sqrt(2), $this->value->getSignedRational()));
        }
    }
}
